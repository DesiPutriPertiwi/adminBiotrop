...
class UWeedController extends Controller
{
	public function index(){
		$herbar = DB::table('speciment_herbarium')
			...
			->select(
 			...
 			->paginate(10);
			// $herbarium = $herbar->where('id_label', '1')->where('status','2');
			$terbaru = $herbar->where('status', 2)->where('type_herbarium', 1);
 		return view('user/herbarium/weed', ['herbariums' => $terbaru, 'herbar' => $herbar]);
	...
	public function search(Request $request) {
			 ...
			 $herbariums = $this->doSearchingQuery($constraints);
			 return view('user/herbarium/weed', ['herbariums' => $herbariums, 'searchingVals' => $constraints]);
		}
